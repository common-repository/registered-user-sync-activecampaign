<?php
/*
Plugin Name: User Sync ActiveCampaign
...
class RUSActiveCampaign {
    ...
    function __construct(){
        ...
        add_action('rusac_add_new_address', array($this, 'rusac_add_new_address'));
        add_action('rusac_edit_address', array($this, 'rusac_update_profile_trigger'));
        add_action('rusac_delete_address', array($this, 'rusac_delete_profile_trigger'));
        ...
        add_action( 'plugins_loaded', array($this, 'rusac_load_plugin_textdomain') );
    }

    function rusac_load_plugin_textdomain() {
    	load_plugin_textdomain( 'rus-activecampaign', FALSE, basename( dirname( __FILE__ ) ) . '/languages/' );
    }
    ...
    function rusac_send_data_to_ac($user_id){
        ...
        if ( !function_exists('curl_init') ){ 
            $err_msg = __("CURL not supported. (introduced in PHP 4.0.2)", "rus-activecampaign");
            echo $err_msg;
            error_log($err_msg);
            return;
        }
        ...
        if ( $params['api_output'] == 'json' && !function_exists('json_decode') ) {
            $err_msg = __("JSON not supported. (introduced in PHP 5.2.0)", "rus-activecampaign");
        	error_log($err_msg);
            echo $err_msg;
            return;
        }
        ...
        if ( !$response ) {
            $err_msg = __("Nothing was returned. Do you have a connection to Email Marketing server?", "rus-activecampaign");
        	error_log($err_msg);
            return;
        }
    ...
    function rusac_add_new_address($user_id) {
        $result = $this->rusac_send_data_to_ac($user_id);
        ...
        if (!empty($result['result_code']) && $result['result_code'] == 1) {
            ...
            if (isset($result[0]['id'])) {
                ...
                update_user_meta( $user_id, '_rusac_synced_user_id', $active_list_id);
            }
            ...
            $active_list_id = $result[0]['id'];
            update_user_meta( $user_id, '_rusac_synced_user_id', $active_list_id);
            $this->rusac_update_profile_trigger($user_id);
    ...
    function rusac_query_to_active_campaign($params, $post=array()){
        ...
        // This sample code uses the CURL library for php to establish a connection,
        // submit your request, and show (print out) the response.
        if ( !function_exists('curl_init') ) die(__("CURL not supported. (introduced in PHP 4.0.2)", "rus-activecampaign"));
    ...
    function rusac_delete_profile_trigger($user_id) {
    	$contact_id = get_user_meta( $user_id, '_rusac_synced_user_id', true);
    	if ($contact_id) {
    		$params = $this->rusac_load_api_details("contact_delete");
	    	$params["id"] = $contact_id;
	        
	        $result = $this->rusac_query_to_active_campaign($params);
            $contact_id = $result['id'];
            update_user_meta( $user_id, '_rusac_synced_user_id', $result['id']);
            update_user_meta( $user_id, '_rusac_synced_deleted', 1);
    	} else {
    		error_log(__("The contact you want to delete is not synced yet or may have been deleted.", "rus-activecampaign"));
    		return false;
    	}
    	return true;
    }
